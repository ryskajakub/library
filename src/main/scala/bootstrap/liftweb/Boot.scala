...
class Boot {
  ...
  val allBorrows = Menu("allBorrows", S ? "allBorrows") / "user" / "allBorrows" >> LocGroup("user")
  ...
  def boot {
    ...
    LiftRules.setSiteMap(SiteMap(index, librarianActions, createLibrary, createLibrarian
      , newStuff, scanning, scanned, newBookCopy, allBooks, allPerson, publicBrowse, allBorrows))
